...
public class Queen extends AbstractFigure {
    ...
    public Pair<Boolean, String> isValidMove(Board[][] chessboard, int[] moves) {
        ...
        if (isOutOfBoundaries(sourceRow, sourceColumn, destinationRow, destinationColumn)) {
            return new Pair<>(false, "Wrong source and destination queen move: " + Arrays.toString(moves));
        }
        ...
        return new Pair<>(false, "Invalid queen move " + Arrays.toString(moves));
    }
    ...
    private Pair<Boolean, String> isClearPath(Board[][] chessboard, int[] moves) {
        ...
        if (sourceRow == destinationRow) {
            ...
            if (isPathNotClearStraight(chessboard, sourceColumn, destinationColumn, sourceRow, true)) {
                return new Pair<>(false, "Invalid queen horizontal move " + Arrays.toString(moves));
            }
        } else if (sourceColumn == destinationColumn) {
            // Clear vertically
            if (isPathNotClearStraight(chessboard, sourceRow, destinationRow, sourceColumn, false)) {
                return new Pair<>(false, "Invalid queen vertical move " + Arrays.toString(moves));
            }
        } else {
            // Clear diagonally
            if (isPathNotClearDiagonal(chessboard, sourceRow, sourceColumn, destinationRow, destinationColumn)) {
                return new Pair<>(false, "Invalid queen diagonal move " + Arrays.toString(moves));
            }
        }
        ...
        if (!destinationMoveCheck) {
            return new Pair<>(false, "Invalid queen destination move " + Arrays.toString(moves));
        }
